...
function getCstLabel(source: string) {
  switch (source) {
    case "informado":
      return { text: "Informado pelo usuario", icon: CheckCircle, color: "text-success" };
    case "sugerido":
      return { text: "Sugerido pelo sistema", icon: Info, color: "text-primary" };
    case "ajustado":
      return { text: "Ajustado apos validacao", icon: AlertTriangle, color: "text-warning" };
    default:
      return { text: "Nao identificado", icon: XCircle, color: "text-muted-foreground" };
  }
}
...
function getGroupConsistencyLabel(status: string | undefined) {
  switch (status) {
    case "coerente":
      return { text: "Grupo coerente com a descricao", color: "text-success" };
    case "divergente":
      return { text: "Grupo informado diverge da descricao", color: "text-warning" };
    case "inferido":
      return { text: "Grupo inferido automaticamente", color: "text-primary" };
    default:
      return { text: "Grupo nao identificado", color: "text-muted-foreground" };
  }
}
      ...
      <CardContent className="space-y-5">
        <div className="rounded-lg bg-secondary/70 border border-border p-4 space-y-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm font-semibold text-foreground">Situacao de aplicacao desta resposta</p>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
            <span className="text-muted-foreground">Estado:</span>
            <span className="text-foreground font-medium">Sao Paulo</span>
            ...
            <span className="text-foreground font-medium">
              {result.application_context || "Emissao conforme regras parametrizadas da base atual"}
            </span>
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            Esta resposta vale exclusivamente para o contexto acima. Operacoes interestaduais ou com outro perfil podem ter regras diferentes.
          </p>
        </div>
        ...
        {isLowConfidence ? (
          <div className="text-center py-6 space-y-3">
            ...
            <p className="text-foreground font-medium">
              Nao foi possivel sugerir um cBenef com seguranca com base nos dados informados.
            </p>
            <p className="text-sm text-muted-foreground">{result.explanation}</p>
            {result.cst_warning && <p className="text-sm text-warning">{result.cst_warning}</p>}
            <p className="text-sm text-muted-foreground">
              Verifique os dados informados ou consulte a legislacao vigente do estado de Sao Paulo.
            </p>
            ...
            {hasSemanticInfo && (
              <div className="mt-2 text-left rounded-lg bg-secondary/50 p-3 space-y-1">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                  Tipo fiscal identificado na descricao
                </p>
                <p className="text-sm text-foreground">
          ...
          <>
            <div className="rounded-lg bg-secondary p-4 space-y-3">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">Classificacao fiscal sugerida</p>
                </div>
                {getConfidenceBadge(result.confidence_level, result.confidence_score)}
              </div>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div>
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-0.5">%ICMS saida</p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                <InfoBlock
                  label="ST Aplicavel"
                  value={result.output_st_applicable === true ? "Sim" : result.output_st_applicable === false ? "Nao" : "—"}
                />
            ...
            {hasSemanticInfo && (
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <InfoBlock label="Familia" value={formatLabel(result.product_family)} />
                  <InfoBlock label="Tipo" value={formatLabel(result.product_type)} />
                  <InfoBlock label="Apresentacao" value={formatLabel(result.presentation_type)} />
                </div>
              </div>
            )}
            ...
            {result.decision_reason && (
              <div className="rounded-lg border border-border p-4 space-y-2">
                ...
                {result.excluded_keywords_hit && result.excluded_keywords_hit.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    <span className="text-xs text-muted-foreground">Excluidas:</span>
            ...
            {(result.legal_basis_name || result.legal_basis_summary) && (
              <div className="rounded-lg border border-border p-4 space-y-2">
                ...
                {result.legal_basis_name && <InfoBlock label="Norma / Referencia" value={result.legal_basis_name} />}
                ...
                <div className="flex flex-wrap gap-4 text-xs text-muted-foreground pt-1 border-t border-border mt-2">
                  {result.rule_version && (
                    <span>Versao: {result.rule_version.version_code || result.rule_version.version_label}</span>
                  )}
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" />
                    Atualizacao: {new Date(result.last_updated_at).toLocaleDateString("pt-BR")}
                  </span>
                </div>
              </div>
            )}
